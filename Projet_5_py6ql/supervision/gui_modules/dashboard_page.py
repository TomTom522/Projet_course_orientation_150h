...
class DashboardPage(QWidget):
    ...
    def __init__(self):
        ...
        self.card_deco = StatusCard("Passerelle USB\nEn attente") 
        self.card_batt = StatusCard("Alerte Batterie\nOK") 
        self.card_move = StatusCard("Alerte Mouvement\nOK")
        
        self.left_grid.addWidget(self.card_deco, 0, 1)
        self.left_grid.addWidget(self.card_batt, 1, 1)
        self.left_grid.addWidget(self.card_move, 2, 1)
        ...
        vbox1 = QVBoxLayout()
        vbox1.addWidget(QLabel("Dernieres coordonnees", styleSheet="font-weight:bold; color:#334155"))
        ...
        bottom_layout.addLayout(vbox2, stretch=2)
    ...
    def charger_balises_api(self):
        ...
        for index in range(lignes_a_afficher):
            if index < len(balises_recues):
                ...
                card = StatusCard(f"{nom}\n({lora_id})\nHors ligne")
                self.balise_cards[lora_id] = {"widget": card, "nom": nom}
            else:
                card = StatusCard(f"Balise {index+1}\n(Non configuree)")
                card.status_bar.setStyleSheet("background-color: #cbd5e1; border-radius: 2px; border: none;")
                
            self.left_grid.addWidget(card, index, 0)
            self.current_balise_widgets.append(card)
    ...
    def update_dashboard_data(self, lat, lon, bat_level, balise_id):
        ...
        if lat is not None and lon is not None:
            ...
            if self.ancrage_position is None:
                ...
                self.card_move.set_status(True)
                self.card_move.lbl_text.setText("Ancrage Fixe")
            else:
                dist = haversine_distance(self.ancrage_position[0], self.ancrage_position[1], lat, lon)
                if dist > self.distance_limite:
                    self.card_move.set_status(False)
                    self.card_move.lbl_text.setText(f"ALERTE DEPLACE\n({int(dist)} m)")
                    ...
                    self.card_move.set_status(True)
                    self.card_move.lbl_text.setText(f"Stable ({int(dist)}m)")
                    self.alertes_mouvement[str_id] = False

        if bat_level is not None:
            if bat_level < 30:
                self.card_batt.set_status(False)
                self.card_batt.lbl_text.setText(f"Batterie Faible\n{bat_level}%")
                ...
                self.card_batt.set_status(True)
                self.card_batt.lbl_text.setText(f"Batterie OK\n{bat_level}%")
